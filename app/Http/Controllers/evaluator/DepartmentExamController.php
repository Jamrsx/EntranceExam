...
class DepartmentExamController extends Controller
{
    ...
    public function examResults(Request $request)
    {
        ...
        if ($testResult) {
            ...
            try {
                ...
                Log::info('Debug examinee relationship', [
                    ...
                    'examinee_name' => $examinee ? $examinee->name : 'null'
                ]);
            } catch (\Exception $e) {
        ...
        $mapped = $results->getCollection()->map(function ($result) {
            ...
            return [
                'id' => $result->id,
                'student_name' => $examinee ? $examinee->name : ('Student ID: ' . $result->examinee_id),
    ...
    public function showExamResult($id)
    {
        ...
        $resultData = [
            'id' => $result->id,
            'student_name' => $result->examinee->name ?? 'N/A',
    ...
    public function update(Request $request, $id)
    {
        ...
        $exam->update($request->only(['exam_title', 'exam_ref_no', 'status', 'time_limit']));
