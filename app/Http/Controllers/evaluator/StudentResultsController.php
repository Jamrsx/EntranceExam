...
class StudentResultsController extends Controller
{
    public function index(Request $request)
    {
        ...
        $query = ExamResult::with(['examinee', 'exam'])->where('is_archived', 1)->latest();
        ...
        $query->whereExists(function ($subQuery) use ($department) {
            $subQuery->select(DB::raw(1))
                ...
                ->where('c.course_code', 'like', '%' . $department . '%');
        });
        ...
        $recommendations = $results->through(function ($result) use ($departmentRecommendations) {
            ...
            $recommendedCourse = $deptRecs->first() ? $deptRecs->first()->course_name : 'N/A';

            return [
                'id' => $result->resultId,
                'name' => $result->examinee->name ?? 'N/A',
        ...
        if (!empty($resultIds)) {
            $personalityDistribution = DB::table('examinee_recommendations as er')
    ...
    public function show($id)
    {
        ...
        return Inertia::render('Evaluator/StudentResultDetail', [
            ...
            'recommendation' => [
                ...
                'examinee_name' => $recommendation->examinee->name ?? 'N/A',
    ...
    public function verifyStudent($id)
    {
        ...
        $verification = [
            'student_name' => $recommendation->examinee->name ?? 'N/A',
            'recommended_course' => $recommendation->recommendedCourse->course_name ?? 'N/A',
    ...
    public function export(Request $request)
    {
        ...
        $callback = function() use ($recommendations) {
            ...
            foreach ($recommendations as $recommendation) {
                fputcsv($file, [
                    $recommendation->examinee->name ?? 'N/A',
